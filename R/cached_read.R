...
#' To skip this, or force reading from scratch, set `skip_file_info=TRUE` or `force=TRUE`, respectively.
#'
...
cached_read <- function(files, label, read_fn,
                        cache = NULL, type = NULL, skip_file_info = FALSE, force = FALSE) {
  .cache <- file_cache(cache = cache, type = type)

  # Option 1: If skipping file info, simply call `with_cache`.
  if (skip_file_info) {
    return(
      with_cache(read_fn(files), label = label, cache = .cache, type = type, force = force)
    )
  }
...
cached_read_csv <- function(files, label, read_type = NULL,
  ...
  if (is.null(read_type)) {
    read_fn <- purrr::discard(read_fn_list, is.null)[[1]]
